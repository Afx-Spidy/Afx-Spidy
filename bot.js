...
const {Message, StringSession, Image, Video} = require('./Spidy/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('💖Spidy ser Working💖 ' + config.WORKTYPE + ' new 🍃'));
          if (config.LANG == 'EN' || config.LANG == 'ML') {
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(conn.user.jid, fs.readFileSync("./photo/Spidy.png"), MessageType.image, { caption: `*💙ʜᴇʏ ʙʀᴏ SPIDY sᴇʀ ɪs  ᴡᴏʀᴋɪɴɢ ` + config.WORKTYPE + ` ❤️* \n\n*💝ᴛʜᴀɴᴋʏᴏᴜ ғᴏʀ ᴜsɪɴɢ  SPIDY sᴇʀ ʙᴏᴛ💝* \n\n*💝NAMMAK PEVER AKAM GOOYS 💝** \n\n*😍ᴀᴘᴘᴏ ᴇɴɢᴀɴᴇ ᴘᴏʟɪᴋᴋᴀʟᴇ😍*`});
                } else {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '*◄━⦁💝 Spidy Ser💝⦁━►*' + degisiklikler + '```', MessageType.text
                    ); 
                } 
          }
    });//thanx afnanplk
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ❤SPIDY 𝚂𝙴𝚁💙 ~______~*' +
                                    '\n*SPIDY 𝚂𝙴𝚁 𝙿𝙾𝙻𝙸 𝙰𝙻𝙻𝙴🔰' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            }
