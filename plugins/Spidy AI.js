...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Spidy özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Spidy yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Spidy yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Spidy, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Spidy, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Spidy features. Turn your account into a ai chatbot!'
    already_on = 'Spidy artificial intelligence is already fully functional.'
    already_off = 'Spidy artificial intelligence is currently running semi-functional.'
    succ_on = 'Spidy Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Spidy Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Abu സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Spidy കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Spidy AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Spidy പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Spidy സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'abu ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.abu on / off' }, (async (message, match) => {
    var maalu_status = `${conf.TALKING_SPIDY}`
    if (match[1] == 'on') {
